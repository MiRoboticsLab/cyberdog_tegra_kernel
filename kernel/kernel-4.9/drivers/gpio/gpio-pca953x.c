				   ...
				   const struct i2c_device_id *i2c_id)
{
	...
	//reset
	chip->reset_gpio = of_get_named_gpio(np, "reset-gpio", 0);
	if (gpio < 0) {
		pr_err("ERROR: %s: no reset-gpio provided\n",
			__func__);
		return -1;
	} else {
		pr_err("%s: reset gpio provided ok, is about to do reset!\n", __func__);
		if(!reset_done) {
			gpio_set_value(chip->reset_gpio, 1);
			mdelay(20);
			gpio_set_value(chip->reset_gpio, 0);
			mdelay(20);
			gpio_set_value(chip->reset_gpio, 1);
			reset_done = true;
		} else {
			pr_err("%s: this is second expander, reset have done in before!\n", __func__);
		}
	}
...
void pca953x_shutdown(struct i2c_client *client)
{
	struct pca953x_chip *chip = i2c_get_clientdata(client);
